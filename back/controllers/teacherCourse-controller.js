const { addTeacherCourse, getCoursesForTeacher } = require('../services/teachercourse-service');
...
async function getTeacherEnrolledCourses(req, res) {
  ...
  try {
    if (user.role === 'admin' || (user.role === 'teacher' && user.id === parseInt(teacherId, 10))) {
...
module.exports = {
  addTeacherCourseController, getTeacherEnrolledCourses,
};
