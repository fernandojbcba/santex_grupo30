  ...
  implements OnInit, OnChanges, AfterViewInit
{
  ...
  attendanceFunction(userId: any, courseId: any, statusId: any) {
    ...
    if ((this.courseDate.courseStateId == 2)) {
      ...
      this.toastService.presentToast('curso no iniciado o finalizado');
    }
  }

  calificar(userId: number, courseId: number, approvalStatus: ApprovalStatus) {
    if ((this.courseDate.courseStateId == 2) ||(this.courseDate.courseStateId == 3) ) {
      this.courseService
        .calificacionUsuarioEnCurso<any>(userId, courseId, approvalStatus)
        .subscribe(
          (response) => {
  ...
  getApprovalStatusLabel(approvalStatusId: number): string {
    const status = this.approvalStatuses.find(
      (item) => item.value === approvalStatusId
    );
